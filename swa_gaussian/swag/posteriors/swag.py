...
class SWAG(torch.nn.Module):
    def __init__(
        ...
        self.base = base
        print("init param len: ", len(list(self.base.parameters())))
        self.base.apply(
            lambda module: swag_parameters(
                module=module, params=self.params, no_cov_mat=self.no_cov_mat
            )
        )
        print("final param len: ", len(list(self.base.parameters())))
    ...
    def collect_model(self, base_model):
        for (module, name), base_param in zip(self.params, base_model.parameters()):
            ...
            # first moment
            # print(mean.shape, base_param.data.shape)
            if (mean.shape != base_param.data.shape):
                continue
